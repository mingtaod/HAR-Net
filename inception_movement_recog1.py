...
# checkpoint = ModelCheckpoint(filepath='./model/best.ckpt', monitor='val_acc', verbose=0, save_best_only='True', mode='auto', period=1)
...
def inception_net(input_shape=(1,128,9),classes=6):
    ...
    # X = BatchNormalization(epsilon=1e-06, mode=0, axis=2, momentum=0.9, weights=None, beta_init='zero', gamma_init='one')(X)
    X = SeparableConv2D(filters=64, kernel_size=(1,1), strides=(filter_stride,1), padding='same', depth_multiplier=1, activation='relu',
    ...
    # X1 = BatchNormalization(epsilon=1e-06, mode=0, axis=2, momentum=0.9, weights=None, beta_init='zero', gamma_init='one')(X1)
    X1 = SeparableConv2D(filters=64, kernel_size=(5,1), strides=(filter_stride,1), padding='same', depth_multiplier=1, activation='relu',
    ...
    # X2 = BatchNormalization(epsilon=1e-06, mode=0, axis=2, momentum=0.9, weights=None, beta_init='zero', gamma_init='one')(X2)
    X3 = SeparableConv2D(filters=64, kernel_size=(13,1), strides=(filter_stride,1), padding='same', depth_multiplier=1, activation='relu',
    ...
    # X3 = BatchNormalization(epsilon=1e-06, mode=0, axis=2, momentum=0.9, weights=None, beta_init='zero', gamma_init='one')(X3)
    X4 = SeparableConv2D(filters=64, kernel_size=(17,1), strides=(filter_stride,1), padding='same', depth_multiplier=1, activation='relu',
    ...
    # X4 = BatchNormalization(epsilon=1e-06, mode=0, axis=2, momentum=0.9, weights=None, beta_init='zero', gamma_init='one')(X4)
    X5 = SeparableConv2D(filters=64, kernel_size=(21,1), strides=(filter_stride,1), padding='same', depth_multiplier=1, activation='relu',
    ...
    # X5 = BatchNormalization(epsilon=1e-06, mode=0, axis=2, momentum=0.9, weights=None, beta_init='zero', gamma_init='one')(X5)
    X7 = SeparableConv2D(filters=64, kernel_size=(29,1), strides=(filter_stride,1), padding='same', depth_multiplier=1, activation='relu',
    ...
    # X_collection = Dropout(0.5)(X_collection)
    # X_collection = BatchNormalization(epsilon=1e-06, mode=0, axis=-1, momentum=0.9, weights=None, beta_init='zero', gamma_init='one')(X_collection)
    # X_collection = Dense(512, activation='relu', kernel_initializer=glorot_uniform(seed=921212))(X_collection) ###
    # X_collection = Dropout(0.4)(X_collection)


    # X_collection = concatenate([X_collection, X_aux_input])


    # X_collection = Dense(256, activation='tanh', kernel_initializer=glorot_uniform(seed=921212))(X_collection)
...
checkpoint = ModelCheckpoint(filepath='./model/best_weights',monitor='val_acc',mode='auto' ,save_best_only='True')
...
# adam = keras.optimizers.SGD(lr=0.001, momentum=0.9, decay=0.01)
model = inception_net(input_shape=(1,128,9), classes=6)
model.compile(loss="categorical_crossentropy", optimizer=adam, metrics=["accuracy"])
model.fit([train_x, train_aux], train_y, epochs=128, batch_size=128,validation_data=([test_x, test_aux], test_y), verbose=2, shuffle=True, callbacks=[history, checkpoint])
# model.save('./model/best_weights.h5')
